...
class KnnClassifier():
    ...
    def _euclidean_distance(self, p: np.ndarray, q: np.ndarray):
        """Computation of the Euclidean distance between to data points."""
        return np.linalg.norm(p - q)

    def _k_nearest_neighbors(self, X_new: np.ndarray):
        """Computation of the k-nearest neighbors of one or more sample x-data."""
        knn_idx_array = np.array([np.argsort([self._euclidean_distance(p_i, q_i) for q_i in self.X])[:self.k] for p_i in X_new])
        return knn_idx_array

    def _class_majority_voting(self, knn_idx: np.ndarray):
        """Choose the class for one or more data points by taking this class which occured most
    ...
    def predict(self, X_new: np.ndarray):
        """Predict the class for one or more new data points."""
